...
window.addEventListener('DOMContentLoaded', () => {
    ...
    layoutContainer.addEventListener('tab-created', ( { detail } ) => {
        ...
        let tabElement = document.getElementById(detail.tabSelector);

        let updateCount = 0;

        let btn = document.createElement('span');
        btn.innerText = `${updateCount}`;
        
        btn.addEventListener('click', () => {
            fin.InterApplicationBus.send({ uuid: fin.me.identity.uuid, name: detail.name},
                `tab-generated-event/${detail.name}`, 'It was clicked');
        });

        fin.InterApplicationBus.subscribe({ uuid: fin.me.identity.uuid }, `content-generated-event/${detail.name}`, () => {
            updateCount++;
            btn.innerText = `${updateCount}`;
        });
